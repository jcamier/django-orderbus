"""Google Pub/Sub pull messages script - pulls existing messages without streaming."""
import json
import logging
import os
...
from google.api_core.exceptions import AlreadyExists, NotFound, PermissionDenied, DeadlineExceeded
...
try:
    ...
    if not response.received_messages:
        print("No messages available in the subscription.")
        print("\nNote: Messages may have already been consumed.")
        print("Try running publisher.py to publish a new message, then run this script again.")
    else:
        ...
        for i, received_message in enumerate(response.received_messages, 1):
            ...
            try:
                ...
                print(f"  {received_message.message.data}")
    ...
    print("\n" + "=" * 60)
    print("⏱️  No messages available (timeout)")
    print("=" * 60)
    print("\nThe subscription doesn't have any messages waiting.")
    print("\nThis could mean:")
    print("  1. No messages have been published yet")
    print("  2. Messages were published before this subscription was created")
    print("  3. All messages have already been consumed")
    print("\nTo see messages:")
    print("  1. Run 'python publisher.py' to publish a new message")
    print("  2. Then run this script again to pull it")
    print("\n" + "=" * 60)
except NotFound as e:
    print(f"\nERROR: Subscription not found: {e}")
    print("\nMake sure the subscription exists before pulling messages.")
except Exception as e:
